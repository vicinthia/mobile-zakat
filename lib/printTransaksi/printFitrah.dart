...
//void main() => runApp(PrintFitrah());

class PrintFitrah extends StatefulWidget {
  final kode_zakat;
  final nama_muz;
  final alamat_muz;
  final nama_amil;
  final total_zakatUang;
  final total_zakatBeras;
  final tgl_zakat;
  ...
  PrintFitrah({
    ...
    this.alamat_muz,
    this.tgl_zakat,
    this.total_zakatUang,
    this.total_zakatBeras,
  });
  @override
  State<PrintFitrah> createState() => _PrintFitrahState();
}

class _PrintFitrahState extends State<PrintFitrah> {
  ...
  void initState() {
    ...
    getDevices();
    print("nominal " + widget.total_zakatUang);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
            ...
            title: Text('Print Bukti Zakat Fitrah'),
            leading: IconButton(
              ...
              return ListTile(
                ...
                onTap: () async {
                  ...
                  if ((await printer.isConnected)) {
                    ...
                    if (widget.total_zakatBeras != null &&
                        widget.total_zakatBeras != "") {
                      printer.printLeftRight(
                        'Fitrah(Beras) :',
                        '${widget.total_zakatBeras} Kg',
                        1,
                      );
                    }

                    if (widget.total_zakatUang != null &&
                        int.tryParse(widget.total_zakatUang) != null) {
                      printer.printLeftRight(
                        'Fitrah(Uang) :',
                        '${CurrencyFormat.convertToIdr(int.tryParse(widget.total_zakatUang), 0)}',
                        1,
                      );
                    }
                    printer.printCustom('================', 2, 1);
